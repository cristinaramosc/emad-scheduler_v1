...
export default function App() {
  ...
  const [assignmentDraft, setAssignmentDraft] = useState({ teacher: "", subject: "", group: "", weekly_hours: "" });
  const [assignmentEdit, setAssignmentEdit] = useState(null);
  const [assignmentEditValues, setAssignmentEditValues] = useState({ teacher: "", subject: "", group: "", weekly_hours: "", allowed_session_lengths: "" });
                        ...
                        <td>
                          {assignmentEdit === a.id ? (
                            <select
                              ...
                              onChange={(event) => setAssignmentEditValues({ ...assignmentEditValues, teacher: event.target.value })}
                            >
                              <option value="">Selecciona un professor</option>
                              {teachers.map((t) => (
                    ...
                    <tr>
                      <td>
                        <select
                          ...
                          onChange={(event) => setAssignmentDraft({ ...assignmentDraft, teacher: event.target.value })}
                        >
                      ...
                      <td>
                        <button onClick={async () => {
                          ...
                          if (res.ok) {
                            setAssignmentDraft({ teacher: "", subject: "", group: "", weekly_hours: "" });
                            await refreshAcademicLists();
                          } else {
      ...
      <section className="scheduler-layout">
        <div className="timetable-controls">
          ...
          {Object.entries(visibleActivitiesBySlot).map(([slotKey, slotActivities]) => {
            ...
            // `activity.duration` ve del backend en HORES (p.ex. 1 = 1h), però
            // cada fila de la graella representa un bloc de 30 min, per tant
            // cal convertir hores -> blocs de 30 min abans de calcular el span.
            const maxDurationBlocks = Math.max(
              2, // mínim una hora = 2 blocs de 30 min
              ...slotActivities.map((activity) => Math.round((Number(activity.duration) || 1) * 2))
            );
